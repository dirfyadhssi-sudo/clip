...
import yt_dlp
import os, tempfile, uuid, threading, time, glob, subprocess, shutil
...
def run_download(job_id, url, start, end, fmt):
    ...
    try:
        ...
        update("running", 30, "Mengunduh dari YouTube...")
        ...
        for line in proc.stdout:
            ...
            if "[download]" in line and "%" in line:
                try:
                    ...
                    mapped = 30 + int(pct * 0.5)
                    update("running", mapped, f"Mengunduh... {pct:.0f}%")
                except:
                    pass
            elif "[Merger]" in line or "Merging" in line:
                update("running", 82, "Menggabungkan video & audio...")
            elif "[ffmpeg]" in line or "Cutting" in line:
                update("running", 90, "Memotong segmen...")

        proc.wait()

        if proc.returncode != 0:
            raise Exception("yt-dlp gagal. Cek URL atau koneksi internet.")

        update("running", 95, "Menyiapkan file...")
